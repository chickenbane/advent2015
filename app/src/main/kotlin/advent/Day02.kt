...
All numbers in the elves' list are in feet. How many total square feet of wrapping paper should they order?
    ...
    val inputLines: List<String> by lazy { FileUtil.resourceToList("input2.txt") }

    data class Dim(val l: Int, val w: Int, val h: Int) {
        private val areas = listOf(l * w, w * h, h * l)
        private val minArea: Int = areas.min()!!
        val wrappingPaperRequired: Int = areas.map { it * 2 }.sum() + minArea

        private val perimeters = listOf(l + l + w + w, w + w + h + h, h + h + l + l)
        private val minPerimeter: Int = perimeters.min()!!
        val ribbonRequired: Int = minPerimeter + (l * w * h)

    }

    fun input2Dim(input: String): Dim {
        val lwhList: List<Int> = input.split("x").map { it.toInt() }
        require(lwhList.size == 3)
        return Dim(lwhList[0], lwhList[1], lwhList[2])
    }
