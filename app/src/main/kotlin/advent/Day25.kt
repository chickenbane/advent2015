...
Actually, since result is dependent on previous, just keep calculating.  So do it the other way:
1. fun(index) -> row, col
2. while(true) { next, if fun(index) @ row, col: break }
"""

    val first: Long = 20151125

    fun codeNum(row: Int, col: Int): Int {
        require(col < row)
        require(row > 0 && col > 0)
        var index = 1
        return 0
    ...
    fun next(prev: Long): Long = (prev * 252533) % 33554393
...
---+---+---+---+---+---+---+
 ...
 1, 4
    ...
    val targetRowCol = RowCol(2947, 3029)


    // This answer seems to take forever.
    fun answerFoo(): Long {
        var idx = 0
        do {
            val rowcol = rowCol(idx)
            println("rowcol=$rowcol")
            idx += 1
        } while (targetRowCol != rowcol)
        println("my idx=$idx")
        return 0L
    }
    ...
    fun answer(): Long {
        ...
        while (true) {
            ...
            code = next(code)
            if (pos == 2) {
                println("code=$code row=$row col=$col")
            }
            if (pos == 5) {
                println("code=$code row=$row col=$col")
            }
            if (row == 2947 && col == 3029) return code
            pos += 1
        }
    }
}